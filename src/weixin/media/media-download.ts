...
// ---------------------------------------------------------------------------
// Types (adapted from openclaw-weixin's inbound.ts)
// ---------------------------------------------------------------------------

/** Fields populated by inbound media download (attached to the message context). */
...
/** Callback to persist a media buffer and return the saved file path. */
...
type MessageItemType_T = WeixinMessage["item_list"] extends (infer T)[] | undefined
  ? T
  : never;
...
export async function downloadMediaFromItem(
  ...
  if (item.type === MessageItemType.IMAGE) {
    ...
    try {
      ...
      errLog(
        `weixin ${label} image download/decrypt failed: ${String(err)}`,
      );
    }
  } else if (item.type === MessageItemType.VOICE) {
    const voice = item.voice_item;
    if (
      (!voice?.media?.encrypt_query_param && !voice?.media?.full_url) ||
      !voice?.media?.aes_key
    )
      return result;
    try {
      ...
      logger.error(
        `${label} voice download/transcode failed: ${String(err)}`,
      );
      errLog(
        `weixin ${label} voice download/transcode failed: ${String(err)}`,
      );
    }
