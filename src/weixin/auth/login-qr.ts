 ...
 * Key changes: replaced OpenClaw account functions with our storage layer,
 * updated messaging from "OpenClaw" to "Claude Weixin Connect".
 */
import { randomUUID } from "node:crypto";

import { apiGetFetch, apiPostFetch } from "../api/api.js";
import { listAccountIds, loadAccount, saveAccount } from "../../storage.js";
import type { StoredAccount } from "../../storage.js";
...
export async function startWeixinLoginWithQr(opts: {
  ...
  try {
    ...
    logger.info(`Starting WeChat login with bot_type=${botType}`);
    ...
    logger.error(`Failed to start WeChat login: ${String(err)}`);
    return {
...
async function refreshQRCode(
  ...
  logger.info(
    `waitForWeixinLogin: refreshing QR code (${qrRefreshCount}/${MAX_QR_REFRESH_COUNT})`,
  );
  try {
    const qrResponse = await fetchQRCode(FIXED_BASE_URL, botType);
    ...
    try {
      ...
      switch (statusResponse.status) {
        ...
        case "confirmed": {
          ...
          const token = statusResponse.bot_token ?? "";
          ...
          // Save account credentials automatically
          saveAccount({
            accountId,
            token,
            baseUrl,
          ...
          logger.info(
            `✅ Login confirmed! ilink_bot_id=${accountId} ilink_user_id=${redactToken(userId)}`,
          );
          ...
          return {
            ...
            baseUrl,
            userId,
            message: "已将 Claude Weixin Connect 连接到微信。",
          };
        }
      }
    } catch (err) {
