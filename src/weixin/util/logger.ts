...
/**
 * Logger — writes JSON lines to the bridge log file:
 *   <dataDir>/bridge-YYYY-MM-DD.log
 *
 * Adapted from @tencent-weixin/openclaw-weixin logger,
 * simplified to remove OpenClaw framework dependency.
 */

const SUBSYSTEM = "claude-weixin-connect";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["claude-weixin-connect"];
...
/** Set the log directory (must be called before first log write). */
export function setLogDir(dir: string): void {
  _logDir = dir;
}

function resolveLogDir(): string {
  return _logDir || path.join(process.cwd(), "data");
}
...
function resolveMainLogPath(): string {
  ...
  return path.join(resolveLogDir(), `bridge-${dateKey}.log`);
}
